                ...
                <div className="space-y-6">
                    ...
                    <div className="rounded-2xl border bg-card overflow-hidden shadow-sm">
                        <div className="px-4 py-3 bg-gradient-to-r from-violet-50 to-indigo-50 dark:from-violet-950/30 dark:to-indigo-950/30 border-b">
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">5-Node v6.0 Dual Pipeline</p>
                        </div>
                        <div className="p-3 space-y-2.5">
                            {[
                                ...
                                { num: 1, label: 'Time Reasoner', mode: 'ai' as const, what: 'LLM assesses if hours are reasonable with full context', checks: ['Hours vs difficulty, experience, and benchmark', 'Description depth vs claimed hours', 'Blocker impact on time justification', 'First entry leniency, history comparison'] },
                                { num: 2, label: 'Content Validator', mode: 'ai' as const, what: 'LLM evaluates genuine learning/work & topic match', checks: ['Does description actually match the topic?', 'Genuine understanding vs vague fluff?', 'New content vs repeat of prior entries?', 'Global Wisdom: learns from admin corrections', 'Copy-paste penalty after AI scoring'] },
                                { num: 3, label: 'Progress Analyzer', mode: 'ai' as const, what: 'LLM checks completion claims & progress coherence', checks: ['Is claimed progress % realistic for hours invested?', 'Completion justified? Enough ground covered?', 'Progress timeline: steady or suspicious jumps?', 'Pace analysis: hours per % progress'] },
                                { num: 4, label: 'Verdict Agent', mode: 'ai' as const, what: 'LLM synthesizes ALL node findings into final decision', checks: ['Sees all node verdicts + reasoning chains', 'Makes connected APPROVE/FLAG/PENDING decision', 'Copy-paste override: never auto-approve', 'Fallback penalty: prefer FLAG over APPROVE'] },
                            ].map((node) => (
